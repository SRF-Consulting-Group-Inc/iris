...
public class OpMoveGateArm extends OpGateNdorV5 {
	...
	public OpMoveGateArm(GateArmImpl d, User o, GateArmState gas) {
		...
		else if ((gas == GateArmState.WARN_CLOSE)
	...
	/** Estimated minimum time needed to start moving the gate arm
	 * (from sending command until motion starts) */
	long minGateMoveStartMS;

	/** Estimated maximum time needed to move gate arm
	 * (from sending command until motion is complete) */
	long maxGateMoveEndMS;
			...
			throws IOException
		{
			...
			switch (target_state) {
				case CLOSED:
					...
					FutureOp.queueOp(device, delay + 2, op);
